  ...
  useEffect(() => {
   ...
   const fetchDashboardStats = async () => {
    try {
     ...
     const response = await axios.get(`${BASE}/api/student/stats`, {
   ...
   const downloadInternshipAgreement = async (internshipId) => {
    try {
      ...
      const response = await axios.get(
        ...
        `${BASE}/api/student/internships/${internshipId}/agreement`,
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: 'blob'
        }
 ...
 const fetchStudentProfile = async () => {
  ...
  const fetchUpcomingDeadlines = async () => {
    try {
      ...
      const response = await axios.get(`${BASE}/api/student/deadlines`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
    ...
    try {
      ...
      if (applicationsRes.data.success) {
        ...
        try {
          ...
          const savedRes = await axios.get(`${BASE}/api/student/saved-internships`, {
            headers: { Authorization: `Bearer ${token}` }
          });
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-shrink-0">
            <img src={profile?.profile_image_url ? `${BASE}${profile.profile_image_url}?t=${Date.now()}` : `https://ui-avatars.com/api/?name=${profile?.first_name}+${profile?.last_name}&background=6366f1&color=fff&size=100&rounded=true`} alt="Profile" className="w-24 h-24 rounded-xl object-cover ring-4 ring-indigo-100" />
          </div>
