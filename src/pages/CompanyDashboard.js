...
useEffect(() => {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const response = await axios.get(`${BASE}/api/company/dashboard-stats`, {
...
const fetchRecentActivities = async () => {
  try {
    ...
    const applicationsRes = await axios.get(`${BASE}/api/company/applications?limit=5`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    
    // جلب آخر 5 تدريبات منشورة
    //const internshipsRes = await axios.get('http://localhost:5000/api/company/internships?limit=5', {
      const internshipsRes = await axios.get(`${BASE}/api/company/internships?limit=5`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const fetchAnalytics = async () => {
  try {
    ...
    //const response = await axios.get('http://localhost:5000/api/company/analytics', {
      const response = await axios.get(`${BASE}/api/company/analytics`, {
      headers: { Authorization: `Bearer ${token}` }
    });
          ...
          >
            {profile?.cover_image_url && (
              <div className="h-32 overflow-hidden">
                <img 
                  src={`${BASE}${profile.cover_image_url}`}
                  alt="Company Cover"
